...
function speedup_third_party_scripts() {
  // Analytics
  wp_enqueue_script(
    'local-gtag',
    get_stylesheet_directory_uri().'/assets/js/gtag.js',
    [], null, true // last arg=true moves it to footer
  );
  // AdSense
  add_filter('script_loader_tag', function($tag, $handle){
    if($handle==='adsbygoogle') {
      return str_replace('<script','<script async defer',$tag);
    }
    return $tag;
  }, 10, 2);
}
add_action('wp_enqueue_scripts','speedup_third_party_scripts');

add_action('save_post', function($post_ID){
  if (wp_is_post_revision($post_ID)) return;
  wp_schedule_single_event(time()+5, 'my_post_publish_tasks', [$post_ID]);
});
add_action('my_post_publish_tasks', function($post_ID){
  // flush CDN cache, ping indexing, regen thumbnails, etc.
});
...
//add_action('wp_head','insert_mobile_critical',1);
//function insert_mobile_critical(){
//  if(wp_is_mobile()){
//    echo "<style>" . file_get_contents(get_stylesheet_directory() . '/critical-mobile.css') . "</style>";
//  }
//}
...
add_filter('get_avatar_url', function($url, $id_or_email, $args) {
  // if it was pointing to /wp-content/litespeed/avatar/XXX.jpg, send it back to Gravatar
  if (strpos($url, 'wp-content/litespeed/avatar/') !== false) {
    $hash = basename(parse_url($url, PHP_URL_PATH), '.jpg');
    $url = "https://secure.gravatar.com/avatar/$hash?s={$args['size']}&d=mp";
  }
  return $url;
}, 10, 3);
...
// add loading="lazy" to every <img> except the one we preloaded
add_filter('wp_get_attachment_image_attributes','sbs_add_lazy_loading', 10, 3);
function sbs_add_lazy_loading($attr, $attachment, $size){
...
// target & defer specific tagDiv scripts
add_filter('script_loader_tag','sbs_defer_tagdiv_js', 10, 3);
function sbs_defer_tagdiv_js($tag, $handle, $src){
  $defer_handles = [
    'td-to-top',     // tdToTop.js
    'td-ajax-search',// tdAjaxSearch.js
    'tdb-menu',      // tdbMenu.js
    'td-menu',       // tdMenu.js
    'td-smart-sidebar',
    'td-ajax-count',
  ...
  if ( in_array($handle, $defer_handles) ){
    // you can switch to async by replacing defer with async
...
// Function to add fetchpriority="high" if the image is deemed critical (e.g., has the class "entry-thumb")
function add_fetchpriority_to_critical_images( $attr, $attachment, $size ) {
    // Check if our target class exists in the image attributes. Adjust the condition if needed.
    if ( isset( $attr['class'] ) && strpos( $attr['class'], 'entry-thumb' ) !== false ) {
        $attr['fetchpriority'] = 'high';
    }
    return $attr;
}
add_filter( 'wp_get_attachment_image_attributes', 'add_fetchpriority_to_critical_images', 10, 3 );
